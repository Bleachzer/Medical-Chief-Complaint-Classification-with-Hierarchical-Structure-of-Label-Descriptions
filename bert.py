...
if not use_fine_tuned_model:
    ...
    model = classifier_class.from_pretrained(bert_model,cache_dir=cache_dir,num_labels=num_labels)
else:
    print('!!! using home brewed model')
    tokenizer = BertTokenizer.from_pretrained(cache_dir)
    model = classifier_class.from_pretrained(cache_dir, num_labels=num_labels)
model.to(device)
...
def train(train_task_name, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            tmpLoss += loss.detach().cpu().numpy().item()
            if (step+1) % 10 == 0:
                savedLoss.append(tmpLoss/10)
                tmpLoss = 0
...
def save_loss(inputArray):
    """
    acc_list: accuracy result list for each test, [0.1,0.2,0.15]
    epoch_list: e.g. [1,2,3,4,5]
    task_name 
    """
    acc_plt = sns.lineplot(y=inputArray, x=list(range(len(inputArray))))
